...
```
表示逐块统计fullURLId。value_counts()这个方法会统计个数，就是像统计每个类型出现的个数并按照左右显示出来。这里是对fullURLId进行统计。按照sql进行逐块统计，每次统计10000条记录，这时把counts数出来展示如下，这里列出一部分
```.py
...
```
其中pd.concat(counts)表示合并那么多的分统计counts，groupby(level=0)这个表示分组，后面表示求和，展示结果如下
```.py
101002       7776
101003     396612
101004        125
